<%@ Page ClassName="Usuariosedit" Language="C#" CodePage="65001" MasterPageFile="masterpage.master" ValidateRequest="false" AutoEventWireup="true" CodeFile="Usuariosedit.aspx.cs" Inherits="Usuariosedit" CodeFileBaseClass="AspNetMaker9_ControlVehicular" %>
...
<p class="aspnetmaker ewTitle"><%= Language.Phrase("Edit") %>&nbsp;<%= Language.Phrase("TblTypeTABLE") %><%= Usuarios.TableCaption %></p>
<p class="aspnetmaker"><a href="<%= Usuarios.ReturnUrl %>"><%= Language.Phrase("GoBack") %></a></p>
<% Usuarios_edit.ShowPageHeader(); %>
<% Usuarios_edit.ShowMessage(); %>
<form name="fUsuariosedit" id="fUsuariosedit" method="post" onsubmit="this.action=location.pathname;return Usuarios_edit.ValidateForm(this);">
<p />
...
<% if (Usuarios.IdUsuario.Visible) { // IdUsuario %>
	<tr id="r_IdUsuario"<%= Usuarios.RowAttributes %>>
		...
		<td<%= Usuarios.IdUsuario.CellAttributes %>><span id="el_IdUsuario">
<div<%= Usuarios.IdUsuario.ViewAttributes %>><%= Usuarios.IdUsuario.EditValue %></div>
<input type="hidden" name="x_IdUsuario" id="x_IdUsuario" value="<%= ew_HtmlEncode(Usuarios.IdUsuario.CurrentValue) %>" />
</span><%= Usuarios.IdUsuario.CustomMsg %></td>
	</tr>
<% } %>
...
<input type="password" name="x_Password" id="x_Password" value="<%= Usuarios.Password.EditValue %>" size="32" maxlength="32"<%= Usuarios.Password.EditAttributes %> />
